...
class HomeGridLayoutTest {
    ...
    fun `a display change places cells at once, without a slide`() {
        ...
        composeRule.setContent {
            Box(Modifier.requiredSize(800.dp, 600.dp).testTag("grid-root")) {
                HomeGridLayout(geometry = geometry, cells = cells, modifier = Modifier.fillMaxSize()) { id ->
                    Box(Modifier.fillMaxSize().testTag("cell-$id"))
                }
            }
        ...
        val expected = gridLeft + with(composeRule.density) { (7 * (inner.cellDp + inner.gapDp)).dp.toPx() }
        assertEquals("the dock is at column 7 of the inner display from the first frames", expected, early, 1f)
        composeRule.mainClock.advanceTimeBy(2000)
        assertEquals("and stays there", expected, left(), 1f)
    }

    /**
     * #119 review: the remembered positions are pixels; a density change with
     * the same dp geometry must not slide from the old density's pixels.
     */
    @Test
    fun `a density change places cells at once, without a slide`() {
        val cells = listOf("a" to Span(2, 1, 1, 1))
        var scale by mutableStateOf(2f)
        composeRule.setContent {
            CompositionLocalProvider(LocalDensity provides Density(scale)) {
                Box(Modifier.size(400.dp, 600.dp).testTag("grid-root")) {
                    HomeGridLayout(geometry = geometry, cells = cells, modifier = Modifier.fillMaxSize()) { id ->
                        Box(Modifier.fillMaxSize().testTag("cell-$id"))
                    }
                }
        ...
        val early = left()
        composeRule.mainClock.advanceTimeBy(2000)
        assertEquals("placed at once at the new density", left(), early, 1f)
    }
